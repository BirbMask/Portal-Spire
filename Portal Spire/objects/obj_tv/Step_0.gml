if (room == Realtitlescreen || room == Disclaimer || room == rank_room || room == rm_levelselect || room == timesuproom || room == boss_room1 || room == characterselect || room == hub_loadingscreen )
{
    visible = false
...
if (instance_exists(obj_debugcontroller) && (!obj_debugcontroller.showhud))
    visible = false
tvdefault = (sprite_index = spr_tv_open ? spr_tv_defaultopen : spr_tv_default)
image_speed = 0.35
 ...
 if (instance_exists(obj_secretportal) && obj_secretportal.secret == 1 && (!obj_player.isgustavo))
	   ...
	   var _transfo = 1
        ...
        if (!_transfo)
        {
            with (obj_player1)
            {
                if (state == (121 << 0) && sprite_index == spr_crazyrun)
